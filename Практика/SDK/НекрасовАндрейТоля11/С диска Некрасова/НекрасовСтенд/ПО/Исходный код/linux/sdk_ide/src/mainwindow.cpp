...
mainwindow::mainwindow(QWidget* pwgt):QMainWindow(pwgt){
  ...
  action_compile->setIcon(QPixmap(":/compile.png"));
	...
	label = new QLabel("Start addr: 0x", this);
...
void mainwindow::slot_open(){
  QString fullName = QFileDialog::getOpenFileName(this, "Open file", QDir::homePath(), "assembler (*.asm *.lst *.hex)");
...
void mainwindow::slot_new(){
    ...
    if (QFileInfo(fullName).completeSuffix() != "asm")
		fullName = QFileInfo(fullName).absolutePath()+"/"+QFileInfo(fullName).baseName()+".asm";
    textEdit *text = new textEdit(tab_widget);
    text->set_full_name(fullName);
    text->start = "0000";
    text->resize(2000,20000);
...
bool mainwindow::slot_save(){
	...
	if ((tab_widget->currentIndex())>=0){
	    ...
	    if ((QDir::currentPath()+"/drv")==QFileInfo(text->fullName).absolutePath()){
...
void mainwindow::slot_compile(void){
	if ((tab_widget->currentIndex())>=0)
	{
		...
		textEdit *text;
		text = qobject_cast<textEdit*>(tab_widget->currentWidget());
		if (QFileInfo(text->fullName).completeSuffix()!="asm"){
			QMessageBox::warning(this, "SDK-IDE", "This file is not for compile");
			return;
		};
		QProcess *process;
		process= new QProcess;
		QString fullName=text->fullName;
		QString fullName2=text->fullName;
		fullName2.chop(3);
		fullName2 += "hex";
		QDir home(QDir::homePath());
		QString relPath, relPath2;
		relPath=home.relativeFilePath(fullName);
		relPath2=home.relativeFilePath(fullName2);
		relPath = relPath.replace('/','\\');
		relPath2 = relPath2.replace('/','\\');
		relPath = "D:\\"+relPath;
		relPath2 = "D:\\"+relPath2;
		QStringList arg;
		arg<<QDir::currentPath()+"/sys/tasm.bat"<<relPath<<relPath2;
		process->startDetached("dosemu", arg);
	};
}
...
void mainwindow::slot_load(void){
	if ((tab_widget->currentIndex())>=0){
		...
		text = qobject_cast<textEdit*>(tab_widget->currentWidget());
		if (QFileInfo(text->fullName).completeSuffix()!="asm"){
			QMessageBox::warning(this, "SDK-IDE", "This file is not for load to SDK");
			return;
		}
		QString fullName2=text->fullName;
		fullName2.chop(3);
		...
		relPath2 = "D:\\"+relPath2;
		QStringList arg;
		arg<<QDir::currentPath()+"/sys/t167b.bat"<<relPath2<<text->start;
		process->startDetached("dosemu", arg);
	}
}
...
void mainwindow::setTabModified(bool swch){
	...
	QString label = tab_widget->tabText(index);
	if (swch) label = "*"+label;
	else label.remove(0,1);
